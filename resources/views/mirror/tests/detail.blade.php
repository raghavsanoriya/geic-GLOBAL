...
@php
    ...
    $support = [];
    foreach ($test['support'] as $i => $item) { $n = $i + 1; $support[] = [$cms["support_{$n}_title"] ?? $item[0], $cms["support_{$n}_copy"] ?? $item[1]]; }
    $faqs = [];
    foreach ($test['faqs'] as $i => $item) { $n = $i + 1; $faqs[] = [$cms["faq_{$n}_question"] ?? $item[0], $cms["faq_{$n}_answer"] ?? $item[1]]; }
@endphp
...
<main class="td-page">
    ...
    <section class="td-section"><div class="td-wrap td-overview"><div><span class="td-kicker">{{ $cms['overview_kicker'] ?? 'Know your exam' }}</span><h2 class="td-title">{{ $cms['overview_title'] ?? 'Understand the format before you start your preparation.' }}</h2><p class="td-lead">{{ $cms['overview_copy'] ?? $test['overview'] }}</p><p class="td-note">{{ $cms['facts_note'] ?? $test['facts_note'] }}</p><a href="{{ $detailUrl }}#support" class="td-button" style="margin-top: 26px">{{ $cms['overview_cta'] ?? 'See how we help' }}</a></div><div class="td-overview__visual"><div class="td-overview__image"><img src="{{ asset($test['image']) }}" alt="{{ $test['image_alt'] }}" loading="lazy" width="1366" height="685"></div><div class="td-overview__badge"><b>{{ $test['number'] }}</b><span>of 8 test-prep pathways for global education goals</span></div></div></div></section>

    <x-detail-media-gallery :images="$testMedia" id="gallery" eyebrow="{{ $cms['gallery_eyebrow'] ?? 'Preparation in focus' }}" title="{{ $cms['gallery_title'] ?? 'Turn practice into a confident application milestone.' }}" lead="{{ $cms['gallery_copy'] ?? 'Your test result matters most when it is planned around your course, university requirements and chosen intake.' }}" />

    <section class="td-section td-section--soft" id="modules"><div class="td-wrap"><div class="td-centre"><span class="td-kicker">{{ $cms['modules_kicker'] ?? 'Test format' }}</span><h2 class="td-title">{{ $cms['modules_title'] ?? 'The areas you will prepare for.' }}</h2><p class="td-lead">{{ $cms['modules_copy'] ?? 'Every test is different. Knowing the sections first helps you create a study plan that spends time where it will matter most.' }}</p></div><div class="td-modules">@foreach($modules as $index => [$title, $copy])<article class="td-module"><span class="td-module__number">{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}</span><h3>{{ $title }}</h3><p>{{ $copy }}</p></article>@endforeach</div></div></section>

    <section class="td-section td-support-wrap" id="support"><div class="td-wrap"><div class="td-centre"><span class="td-kicker">{{ $cms['support_kicker'] ?? 'Your preparation path' }}</span><h2 class="td-title">{{ $cms['support_title'] ?? 'Build the score plan around your actual application.' }}</h2><p class="td-lead">{{ $cms['support_copy'] ?? 'Your test strategy should work with your university shortlist, document schedule and planned intake — not sit separately from them.' }}</p></div><div class="td-support">@foreach($support as $index => [$title, $copy])<article class="td-support__item"><span class="td-support__number">{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}</span><h3>{{ $title }}</h3><p>{{ $copy }}</p></article>@endforeach</div></div></section>

    <section class="td-section" id="universities"><div class="td-wrap"><div class="td-centre"><span class="td-kicker">{{ $cms['universities_kicker'] ?? 'University network' }}</span><h2 class="td-title">{{ $cms['universities_title'] ?? 'Plan for the institutions on your shortlist.' }}</h2><p class="td-lead">{{ $cms['universities_copy'] ?? 'We help you verify the current test and score expectations for each university before you book or submit your result.' }}</p></div><x-university-network :universities="$testUniversities" country="Global" slug="test-{{ $test['slug'] }}" /></div></section>

    <section class="td-section" id="faqs"><div class="td-wrap td-faq-layout"><div class="td-faq-intro"><span class="td-kicker">{{ $cms['faq_kicker'] ?? 'Common questions' }}</span><h2 class="td-title">{{ $cms['faq_title'] ?? 'Answers before you book.' }}</h2><p class="td-lead">{{ $cms['faq_copy'] ?? 'Requirements vary by university and programme. A counselling conversation can help you verify your test choice before you begin.' }}</p><a href="{{ url('/#contact') }}" class="td-button" style="margin-top: 25px">Talk to a counsellor</a></div><div class="td-faq">@foreach($faqs as [$question, $answer])<details><summary>{{ $question }}</summary><p>{{ $answer }}</p></details>@endforeach</div></div></section>

    <section class="td-section td-section--soft" id="enquire"><div class="td-wrap"><div class="td-cta"><div class="td-cta__content"><span class="td-kicker">{{ $cms['cta_kicker'] ?? 'Start with the right plan' }}</span><h2>{{ $cms['cta_title'] ?? 'Get clear on your '.$test['title'].' preparation and application timeline.' }}</h2><p>{{ $cms['cta_copy'] ?? 'Tell us where you want to study, what you plan to study and your target intake. We’ll help you decide the most practical next step.' }}</p><a href="{{ url('/#contact') }}" class="td-button">{{ $cms['cta_label'] ?? 'Book free counselling' }} <span aria-hidden="true">→</span></a></div></div></div></section>
</main>
