...
@section('content')
    ...
    <section class="panel lead-table">
        ...
        <form class="filters" method="get" action="{{ route('admin.enquiries.index') }}">
            ...
            @if(request()->filled('q') || request()->filled('destination') || request()->filled('source'))<a class="button button--quiet" href="{{ route('admin.enquiries.index') }}">Clear</a>@endif
        </form>
        @if($enquiries->isEmpty())
            <div class="empty"><h3>No enquiries to show</h3><p>New public website enquiries will appear here.</p></div>
        @else
            <div class="table-wrap"><table><thead><tr><th>Student</th><th>Destination</th><th>Study plan</th><th>Source</th><th>Received</th></tr></thead><tbody>@foreach($enquiries as $enquiry)<tr><td><span class="student">{{ $enquiry->full_name }}</span><span class="sub">{{ $enquiry->email }} · {{ $enquiry->phone }}</span></td><td><span class="pill">{{ $enquiry->destination ?: 'Not specified' }}</span><span class="sub">{{ $enquiry->city }}</span></td><td><span class="student">{{ $enquiry->preferred_course ?: 'Course not provided' }}</span><span class="sub">{{ $enquiry->study_level }} · {{ $enquiry->preferred_intake }}</span></td><td><span class="lead-origin {{ $enquiry->source === 'wordpress' ? 'lead-origin--wordpress' : '' }}">{{ $enquiry->source === 'wordpress' ? 'WordPress history' : 'New website' }}</span><span class="sub">{{ $enquiry->source_form ?: $enquiry->source_page }}</span></td><td><span class="student">{{ \Carbon\Carbon::parse($enquiry->created_at)->format('d M Y') }}</span><span class="sub">{{ \Carbon\Carbon::parse($enquiry->created_at)->format('h:i A') }}</span></td></tr>@endforeach</tbody></table></div>
