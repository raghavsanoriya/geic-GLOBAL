...
@section('content')
    ...
    <section class="media-upload-layout">
        <form class="panel media-upload" method="post" action="{{ route('admin.media.store') }}" enctype="multipart/form-data">
            @csrf
            <div class="media-upload__head"><h2>Upload images</h2><p>Select up to 30 assets at once. They will be added to the shared library together.</p></div>
            <div class="media-dropzone" role="button" tabindex="0" aria-label="Drag and drop images or browse files" data-media-dropzone><div><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 18H6a4 4 0 0 1-.7-7.94A7 7 0 0 1 18.8 9.2 4.5 4.5 0 0 1 18.5 18H17"/><path d="m9 12 3-3 3 3M12 9v9"/></svg><strong>Drag &amp; drop images or <span>browse</span></strong><p>JPG, PNG or WebP · maximum 6 MB each · up to 30 files</p></div></div>
            <input class="media-file" id="images" type="file" name="images[]" accept="image/png,image/jpeg,image/webp" required multiple data-media-file>
            ...
            @error('images')<small style="display:block;margin-top:8px;color:#c81420">{{ $message }}</small>@enderror @error('images.*')<small style="display:block;margin-top:8px;color:#c81420">{{ $message }}</small>@enderror
            ...
            <div class="media-upload__fields"><div class="field"><label for="folder">Upload folder</label><input class="input" id="folder" name="folder" value="{{ request('folder', 'General') }}" list="media-folders" maxlength="100" placeholder="General"><datalist id="media-folders"><option value="General">@foreach($folders as $folder)<option value="{{ $folder }}">@endforeach</datalist></div></div>
            <div class="media-upload__actions"><button class="button" type="submit">Upload selected images</button></div>
        </form>
    ...
    <form class="panel" method="post" action="{{ route('admin.media.folders.store') }}" style="display:flex;align-items:end;gap:10px;margin-top:18px;padding:16px"><div class="field" style="flex:1;margin:0"><label for="new-folder">Create folder</label><input class="input" id="new-folder" name="folder" maxlength="100" placeholder="e.g. University logos" required></div>@csrf<button class="button" type="submit">Create folder</button></form>
    <form class="filters" method="get" action="{{ route('admin.media.index') }}" style="margin-top:24px"><input class="input" type="search" name="q" value="{{ request('q') }}" placeholder="Search filename, path or description" aria-label="Search media"><select class="select" name="folder" aria-label="Filter by folder"><option value="">All folders</option>@foreach($folders as $folder)<option value="{{ $folder }}" @selected(request('folder') === $folder)>{{ $folder }}</option>@endforeach</select><select class="select" name="sort" aria-label="Sort media"><option value="newest" @selected(request('sort','newest') === 'newest')>Newest first</option><option value="oldest" @selected(request('sort') === 'oldest')>Oldest first</option><option value="name" @selected(request('sort') === 'name')>Name A–Z</option><option value="size" @selected(request('sort') === 'size')>Largest first</option></select><button class="button" type="submit">Search</button></form>
    ...
    <section class="media-grid" aria-label="Uploaded images">
        @forelse($assets as $asset)
            <article class="media-card"><img src="{{ asset($asset->path) }}" alt="{{ $asset->alt_text ?: '' }}" loading="lazy"><div class="media-card__body"><strong style="display:block;font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ $asset->original_name }}</strong><span style="display:block;color:#7e8da7;font-size:10px">{{ $asset->folder ?: 'General' }} · {{ number_format($asset->size / 1024, 0) }} KB</span><code>{{ $asset->path }}</code></div></article>
        @empty
...
<script>
    (() => {
        ...
        function showFile() { const files = [...(input.files || [])]; if (!files.length) return; const file = files[0]; preview.querySelector('img').src = URL.createObjectURL(file); preview.querySelector('[data-media-file-name]').textContent = files.length === 1 ? file.name : `${files.length} images selected`; preview.querySelector('[data-media-file-size]').textContent = files.length === 1 ? `${(file.size / 1024 / 1024).toFixed(2)} MB` : 'Ready for batch upload'; preview.classList.add('is-visible'); }
